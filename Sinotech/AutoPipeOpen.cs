            ...
            foreach (BeamsWallsPipes bwpValue in beamsWallsPipesList)
            {
                ...
                if (pipeOpen is FamilyInstance && pipeOpen != null)
                {
                    try
                    {
                        ...
                        // 單位轉換
                        double diameter = UnitUtils.ConvertFromInternalUnits(bwpValue.outerDiameter, UnitTypeId.Millimeters);
                        double openSize = OpenSize(diameter);
                        double openingDistance = UnitUtils.ConvertToInternalUnits((openSize - diameter), UnitTypeId.Millimeters) / 2;
