...
namespace Sinotech_2020.FamilyProtect
{
    [Transaction(TransactionMode.Manual)]
    [Regeneration(RegenerationOption.Manual)]
    public class FamilyConversionToDirectShape : IExternalEventHandler
    {
        public Action<List<string>> RefreshListView; // 修改完成後回呼Form刷新用
        public static List<string> successFamilyNames = new List<string>();
        ...
        public void Execute(UIApplication uiapp)
        {
            ...
            using (Transaction trans = new Transaction(doc, "建立模型"))
            {
                ...
                successFamilyNames = new List<string>();
                ...
                foreach (FamilyInstance chooseFamily in chooseFamilys)
                {
                    try
                    {
                        ...
                        try
                        {
                            ...
                            successFamilyNames.Add(chooseFamily.Symbol.FamilyName);
                        }
                        catch(Exception ex) // 如果建立失敗, 改使用BRepBuilder建立Solid
                        {
                            string error = ex.Message + "\n" + ex.ToString(); 
                            
                            resultList = new List<GeometryObject>();
                            foreach (Solid solid in solids)
                            {
                                foreach (Face face in solid.Faces)
                                {
                                    if (face != null)
                                    {
                                        try
                                        {
                                            Solid newSolid = CreateSolidFromFace(face);
                                            resultList.Add(newSolid);
                                        }
                                        catch (Exception) { }
                                    }
                                }
            ...
            successFamilyNames = successFamilyNames.Distinct().ToList(); // 移除重複名稱
            ...
            {
                System.Windows.Forms.Form chooseElemsForm = RefreshListView.Target as System.Windows.Forms.Form;
                if (chooseElemsForm == null || chooseElemsForm.IsDisposed || !chooseElemsForm.IsHandleCreated)
                    System.Diagnostics.Debug.WriteLine("⚠️ Form 尚未準備好，略過更新");
                else
                    RefreshListView?.Invoke(successFamilyNames); // 結束transaction前，呼叫回傳事件
            }
        ...
        private List<Solid> GetSolids(Document doc, Element elem)
        {
            ...
            //List<Solid> solids = GetValidSolids(geomElem);
