...
namespace AutoExport
{
    ...
    public class ExportPDF
    {
        ...
        public static Document m_RevitDocument;
        ...
        public Result ExportToPDF(UIApplication uiapp)
        {
            ...
            {
                // Write to log file
                oMethods.TextFileWrite(strLogfileFullPath, "No values found in: '" + strPDFPrintCFG + "': " + 
                                       oMethods.DateCurrentGet() + "-" + oMethods.TimeCurrentGet(":") + ", Closing Revit", true);
            ...
            // Intiate the events
            m_uiApp = uiapp;
            m_uiApp.Idling += new EventHandler<IdlingEventArgs>(idleUpdate);
            m_uiApp.DialogBoxShowing += new EventHandler<DialogBoxShowingEventArgs>(HandleDialogBoxShowing);
        ...
        public void idleUpdate(object sender, IdlingEventArgs e)
        {
            ...
            try
            {
                ...
                foreach (string strTextLine in strListFromCFG)
                {
                    ...
                    // Write to log file
                    oMethods.TextFileWrite(strLogfileFullPath, "Processing line " + i.ToString() + " in CFG file: " +
                                           oMethods.DateCurrentGet() + "-" + oMethods.TimeCurrentGet(":"), true);
                    ...
                    {
                        // Write to log file
                        oMethods.TextFileWrite(strLogfileFullPath, "5 parameters not found in line: " + i.ToString() + 
                                               " in the CFG file processing : " + oMethods.DateCurrentGet() +
                    ...
                    if (strRVTFullPath.Length > 3)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // Write to log file
                                oMethods.TextFileWrite(strLogfileFullPath, "RVT file not found: '" + strRVTFullPath + "' on line: " + i.ToString() + " : " + 
                                                       oMethods.DateCurrentGet() + "-" + oMethods.TimeCurrentGet(":") + ", Processing next line", true);
                    ...
                    {
                        // Write to log file
                        oMethods.TextFileWrite(strLogfileFullPath, "Could not open file: '" + strRVTFullPath + "' on line: " + i.ToString() + " : " + 
                                               oMethods.DateCurrentGet() + "-" + oMethods.TimeCurrentGet(":") + ", Processing next line", true);
                    ...
                    // Print the PDFs
                    oMethods.PrintToPDF(strRVTFullPath, strSheetSet, strPrintSet, strPDFOutPath, strProcessDate, strLogfileFullPath, i, doc);
                }

                // Write to log file
                oMethods.TextFileWrite(strLogfileFullPath, oDefaultValues.strLogFileDivide, true);
                // Write to log file
                oMethods.TextFileWrite(strLogfileFullPath, "Process End: " + oMethods.DateCurrentGet() + "-" + oMethods.TimeCurrentGet(":"), true);
                // Write to log file
                oMethods.TextFileWrite(strLogfileFullPath, oDefaultValues.strLogFileDblDivide, true);
            ...
            {
                // Write to log file
                oMethods.TextFileWrite(strLogfileFullPath, "Error: '" + ex.Message + " : " + oMethods.DateCurrentGet() + "-" + 
                                       oMethods.TimeCurrentGet(":") + ", Processing stopped", true);
            }
