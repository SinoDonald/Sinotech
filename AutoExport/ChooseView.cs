...
namespace AutoExport
{
    public partial class ChooseView : System.Windows.Forms.Form
    {
        ...
        {
            // 找到"圖紙"或"Sheet"的視圖
            string vftName = (from x in viewInfoList
                              orderby x.vftName
                              where x.vftName.Equals("圖紙") || x.vftName.Equals("Sheet")
                              select x.vftName).Distinct().FirstOrDefault();
            treeView1.Nodes.Add(vftName);
            treeView1.Nodes[0].Checked = true; // 圖紙預設勾選

            // 依圖紙與名稱排序
            List<ViewInfo> viewInfos = (from x in viewInfoList
                                        where x.vftName.Equals(vftName)
                                        select x).OrderBy(x => x.vftName).ThenBy(x => x.name).Distinct().ToList();
            int nodeCount = 0;
            foreach (var viewInfo in viewInfos)
            {
